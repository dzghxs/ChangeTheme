...
public class CheckPermissionsActivity extends BaseActivity
	...
	SharedPreferences.Editor bu;
	SharedPreferences buu;
	...
	protected void onResume() {
		super.onResume();
		bu = getSharedPreferences("quxian",MODE_PRIVATE).edit();
	...
	public void onRequestPermissionsResult(int requestCode,
		...
		if (requestCode == PERMISSON_REQUESTCODE) {
			if (!verifyPermissions(paramArrayOfInt)) {
				buu = getSharedPreferences("quxian",MODE_PRIVATE);
				if(buu.getString("isone","").equals("1")){
					isNeedCheck = true;
				}else{
	...
	private void showMissingPermissionDialog() {
		...
		builder.setTitle("提示");
		builder.setMessage("当前应用缺少必要权限。请点击\\\"setting\\\"-\\\"权限\\\"-打开所需权限。");
